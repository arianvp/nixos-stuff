...
let
  ...
  socketShorthand = {
    ...
    config.systemd.sockets =
      let
        servicesToActivate = lib.filterAttrs (_: value: value.listenStream != [ ]) config.systemd.services;
      in
  ...
  socketProxy = {
    ...
    config.systemd.services = lib.mapAttrs' (
      name: value:
      lib.nameValuePair "systemd-socket-proxy@${utils.escapeSystemdPath name}" {
